...
- Scheduled sync sends Telegram only when new events are created, when new Gmail review items are created, or when a connection first enters an error state. Gmail review notifications are bundled by the legacy integration notifier unless V3 Gmail nudge delivery is live and eligible for that user, in which case the legacy review notification is suppressed to avoid duplicates.
