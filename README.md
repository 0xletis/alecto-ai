...
- Natural Gmail setup/status/list replies show whether Gmail is connected, which tracking rules are active, whether automatic worker sync is enabled, how to run manual sync, and goal-based recommendations for job-search or work-action tracking. Timing questions explain that Gmail is checked on manual sync or scheduled integration sync, not instantly on email arrival. Questions about rules that are "on" show active rules first and hide paused/error records from the normal view.
...
- `/sync_gmail` reports safe user counters, including messages checked, new review items, and new events. If pending email reviews exist, the reply says how many are waiting and points the user to `email reviews`. Natural timing questions such as `when do you check Gmail?`, `do you check Gmail automatically?`, and `will you notify me about emails?` explain manual sync, scheduled sync status, review handling, and that Gmail webhooks/instant arrival tracking are not implemented. `/sync_gmail_debug` adds per-rule IDs, LLM counters, and last errors without email bodies or tokens.
...
- Not implemented yet: Gmail webhooks, full-inbox/all-mail LLM monitoring, per-rule sync schedules, business-hours sync profiles, and goal-specific proactive email-notification preferences.
