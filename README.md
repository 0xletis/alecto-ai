...
Agent Runtime v3 can handle grounded Gmail review follow-ups after a visible review list is shown. It can answer questions about one visible review from stored subject/snippet/evidence only, reconcile mixed numbered triage such as `ignore 1, turn 2 and 3 into tasks, keep 4 in review`, convert one or more visible reviews into ActionItems, parse requested timing such as `5 minutes from now`, `tomorrow morning`, or dates mentioned in the email snippet, create companion pre-due reminder ActionItems when the user asks for reminders, list only scheduled reminders for `do I have any reminders on`, correct a created task's time without duplicating it, and list scheduled meeting-like ActionItems. It never mutates Gmail itself and does not create Calendar events.
...
- After showing the email review inbox, Alecto stores a short-lived visible-number context. Follow-ups such as `show 1`, `does the jobs newsletter mention frontend roles?`, `approve 1`, `reject 2`, `ignore 1`, `keep 4 in review`, `approve all job-search reviews`, `reject all Endesa reviews`, `reject all the rest reviews from Endesa`, `turn 3 into an action tomorrow`, `turn 2 and 3 into tasks 5 minutes from now and remind me at that time`, `turn 2 and 3 into tasks at the time they say and remind me 30 minutes before each`, `do I have any reminders on`, and `brainstorm meeting means 12pm, change it` resolve only against the visible inbox/tasks and only mutate reviews/tasks that are still valid. Mixed numbered review triage is reconciled before execution, so explicitly ignored or kept reviews cannot also become tasks. Expired context asks the user to run `email reviews` again. Visible numbers are not permanent IDs.
