...
CONVERSATION_ORCHESTRATOR_V2_ENABLED=false
LLM_OPERATION_PLANNER_ENABLED=false
LLM_OPERATION_PLANNER_MODEL=gpt-4o-mini
LLM_OPERATION_PLANNER_TIMEOUT_MS=3000
...
Conversation Orchestrator v2 Phase 1 adds a cleaner operation-planning path behind the opt-in `CONVERSATION_ORCHESTRATOR_V2_ENABLED=true` flag on `/messages/process`. (The debug-only `/messages/process_v2` endpoint that used to expose this unconditionally has been retired — it had zero production callers.) It builds a short-lived `ConversationContext`, an `AvailableOperations` catalog, a structured OperationPlanner contract, deterministic validation/executors, and a response composer. Phase 1 owns action hygiene list creation plus its visible-list replies, all-except cleanup batches, recent mutation questions, explicit memory requests, selected operator-attention/read surfaces, conservative progress logging such as CV/application and workout reports, natural confirmation variants, cross-domain visible-context safety, reference-text safety, and betting/trading risk precedence. With `LLM_OPERATION_PLANNER_ENABLED=true` plus `OPENAI_API_KEY` or the test mock env, v2 can call the real LLM operation planner for selected migrated scopes; failures, timeouts, low confidence, or invalid plans fall back to deterministic behavior. The LLM only proposes structured operations from safe summarized context. Deterministic validators and executors still own target checks, confirmations, and every DB mutation. Legacy routing remains active for surfaces not yet migrated. This flag is off in every environment tracked by this repository, but that alone doesn't prove no live deployment has it set — see `docs/09-architecture-inventory.md`'s "Conversation Orchestrator v2 Branch Retirement Audit" before assuming it's safe to remove. See `docs/09-architecture-inventory.md` for the current ownership map and split-brain risks.
...
- `/sync_gmail` reports safe user counters, including messages checked, new review items, and new events. If pending email reviews exist, the reply says how many are waiting and points the user to `email reviews`. Manual sync remains separate from the background schedule and does not reset the next scheduled Gmail check. Natural timing questions such as `when do you check Gmail?`, `do you check Gmail automatically?`, and `will you notify me about emails?` explain manual sync, scheduled sync status, review-notification preference, review handling, and that Gmail webhooks/instant arrival tracking are not implemented. `/sync_gmail_debug` adds per-rule IDs, LLM counters, and last errors without email bodies or tokens.
...
- When `CONVERSATION_ORCHESTRATOR_V2_ENABLED=true`, `/messages/process` can use the v2 operation-planning path for migrated scopes. `LLM_OPERATION_PLANNER_ENABLED=true` lets v2 call the real LLM operation planner for those scopes, but deterministic validation/execution still owns allowed targets, confirmations, and DB mutations. Route debug reports `handledBy`, `v2SkippedReason`, `plannerUsed`, `llmPlannerAttempted`, `llmPlannerUsed`, `llmPlannerFailedReason`, `operationPlanValidated`, `policyPrecheckResult`, `mutationExecuted`, and whether legacy semantic routing was attempted/used.
...
- Scheduled sync sends Telegram only when new events are created, when new Gmail review items are created, or when a connection first enters an error state. Gmail review notifications are bundled, for example `2 Gmail reviews are waiting: 1 job-search, 1 custom tracking. Say "email reviews" to handle them.`
