...
- [~] Conversation Orchestrator v2 Phase 1: `/messages/process_v2` plus opt-in `/messages/process` flag, with `ConversationContext`, `AvailableOperations`, OperationPlanner contract, deterministic validator/executors, response composer, explicit route-debug ownership fields, and message route registration extracted to `apps/api/src/routes/messages.ts`. Runtime remains deterministic by default; `LLM_OPERATION_PLANNER_ENABLED=true` can call the real LLM operation planner for selected migrated scopes while validation/execution stay deterministic. See `docs/09-architecture-inventory.md` for the ownership map and migration plan.
...
- keep migrating surfaces into Conversation Orchestrator v2 and extracting route wiring/executors/context helpers out of `apps/api/src/server.ts`; Phase 1 proves the pattern and now has an optional real LLM planner runtime, but Gmail rule management, planning, daily loop, and generic chat are not fully migrated. Use `docs/09-architecture-inventory.md` as the migration checklist.
