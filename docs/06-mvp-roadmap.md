...
- keep migrating surfaces into Conversation Orchestrator v2 and extracting executors/context helpers out of `apps/api/src/server.ts`; Phase 1 proves the pattern and now has an optional real LLM planner runtime, but Gmail rule management, planning, daily loop, and generic chat are not fully migrated
