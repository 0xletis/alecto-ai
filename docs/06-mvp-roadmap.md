...
- [x] ~~Conversation Orchestrator v2 Phase 1~~ — **retired.** `/messages/process_v2` and the opt-in `CONVERSATION_ORCHESTRATOR_V2_ENABLED=true` flag on `/messages/process`, along with `ConversationContext`, `AvailableOperations`, the OperationPlanner contract, deterministic validator/executors, and response composer, have all been deleted — product confirmed no live deployment relied on the flag. Message route registration stays extracted at `apps/api/src/routes/messages.ts`. **Agent Runtime v3** (`POST /agent/message`) is the default normal-chat runtime today, unrelated to v2. See `docs/09-architecture-inventory.md`'s "Conversation Orchestrator v2 — Full Retirement" for the deletion record.
...
- keep migrating surfaces into Agent Runtime v3's tool catalog (Conversation Orchestrator v2 is retired — this is now the only forward migration target) and extracting route wiring/handlers out of `apps/api/src/server.ts`; Gmail rule management, planning, daily loop, and generic chat are not yet in v3's tool catalog. Use `docs/09-architecture-inventory.md` as the migration checklist.
