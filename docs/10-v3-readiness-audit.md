...
| Guardrails/risk responses | **Partial** | Yes (hard-coded, LLM-classified) | n/a | scripted (13, NEW — documents both halves) | n/a | n/a | **No** | See §5 below — this is the single most important finding in this audit. |
...
## 5. Guardrails — the headline finding
...
**Net effect:** a user typing "I want to bet €1000 because it feels safe" into normal V3 chat today gets **no special handling at all** unless they have already, separately, configured "bet"/"gambling" as a `knownTrigger` — something no default user will have done. The same message through legacy, or through the `/action` slash command's guardrail-text routing, gets a hard, explicit refusal. Scripted scenario 13 proves both halves of this precisely.
...
- `messages-process.ts` itself — cannot be deleted while `TELEGRAM_AGENT_RUNTIME_V3_ENABLED=false` remains a supported escape hatch, `/help` depends on it, and the guardrail-routing for `/action`/`/todo`/`/add_action` depends on it (§5). This is the most load-bearing legacy file in the codebase today, specifically *because* it's the only place the real gambling/trading risk classifier lives.

**Candidates to keep as admin/fallback indefinitely:** `messages-process.ts` (as the `TELEGRAM_AGENT_RUNTIME_V3_ENABLED=false` escape hatch and the only home of the risk classifier, until/unless that's ported into V3) and `gmail-conversation.ts`/`daily-conversation.ts`'s slash-command-serving pieces (no reason to touch working, still-used code paths).

## 7. Early-user readiness verdict

**What Alecto can safely do now, in normal V3 chat:** save/recall memories; log progress and events; full action CRUD plus action-hygiene cleanup; the complete next-week planning propose → edit → confirm loop; weekly review show/save; Gmail connection status, rule listing/creation/pause/resume/archive; daily-loop settings show/change. All of the above are tested (unit and/or scripted), require confirmation where they mutate meaningfully, and never fabricate facts in their grounded replies.

**What it should not promise yet:** managing goals via chat (no create/list/update path — falls back to being logged as a loose memory at best); reliable Gmail-review triage via chat (the tool exists but the planner doesn't know to use it, and its own summary doesn't list subjects); any gambling/financial-risk safety net beyond what a user has manually pre-configured; onboarding/help guidance through normal chat (still slash/legacy-route only, which is fine, just don't imply chat covers it); any proactive/unprompted behavior (doesn't exist yet, by design).

**Biggest risks for user testing:**
1. **Safety regression in normal chat.** A financial/gambling-risk message gets a materially weaker response in V3 chat than the same message would get through legacy or through a slash command — and this is invisible to the user, who has no reason to know V3 chat and `/action` handle risky text differently.
2. **Silent capability gap on Gmail reviews.** A user asking "what needs my attention in my inbox?" is likely to get an unhelpful or vague answer, since the planner has no instruction that `gmail.review.list` exists — unlike every other migrated capability, which explicitly teaches the planner when to reach for it.
3. **Inconsistent goal handling.** "I want to run a marathon" or "what are my goals?" get inconsistent, sometimes-honest, sometimes-silently-inadequate answers, because there's no explicit "goals aren't manageable via chat yet" instruction the way daily-loop and Gmail-rule migrations explicitly teach the planner to decline out-of-scope asks honestly.

**Top 3 blockers before external users:**
1. Add a hard-coded (not user-configured-only) safety check for high-risk topics (gambling/financial risk at minimum) to V3's normal-chat path — either wire in the existing legacy risk classifier or build an equivalent V3-native check.
2. Add `planner.ts` prompt guidance for `gmail.review.list`/`reject`/`to_action` (and give `gmail.review.list`'s summary itemized subjects, mirroring `gmail.rule.list`), so the already-built tool is actually reachable by a real LLM turn.
3. Give the planner explicit instructions to honestly decline goal-management requests it can't fulfill (mirroring the daily-loop/Gmail-rule pattern), or add a minimal read-only `goal.list` tool so "what are my goals?" returns real titles instead of a bare count.

**Top 3 product improvements after baseline:** proactive operator behavior (the next planned phase, which this audit exists to gate); real goal management via chat, once the safety and Gmail-review gaps close; itemized Gmail-review triage in chat ("what needs review?" → subjects listed → "approve the recruiter one").

## 8. Validation

`pnpm typecheck` ✅ · `pnpm build` ✅ · `pnpm test` — **313/313** ✅ (308 before this pass + 5 new scripted scenarios) · `git diff --check` ✅. No product code changed in this pass — only test files and documentation.
