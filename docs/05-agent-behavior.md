...
- [~] Conversation Orchestrator v2 Phase 1 is the new architecture path for context-bound natural operations. It builds a channel-neutral `ConversationContext`, exposes an `AvailableOperations` catalog, validates operation plans, executes through existing services, and composes replies from actual execution results. It owns action hygiene list creation plus cleanup replies when enabled. The planner is deterministic by default; `LLM_OPERATION_PLANNER_ENABLED=true` can route selected migrated scopes through the real LLM operation planner, with timeout/error/invalid-plan fallback to deterministic behavior. Deterministic validators and executors still own all DB mutations. It is currently wired for `/messages/process_v2` and optional `/messages/process` usage through `CONVERSATION_ORCHESTRATOR_V2_ENABLED=true`; legacy routing still owns surfaces not yet migrated. Route debug must show whether v2, LLM planning, legacy semantic routing, policy precheck, validation, and mutation actually happened.
