...
5. ~~`/messages/process` handler body~~ — **audited, not extracted**: confirmed **Classification C (not safe yet)**, see "`/messages/process` Extraction Readiness Audit" below. Every dispatcher in its call graph is either too broad to isolate or blocked by the same `buildWeeklyReviewContext` dependency that blocked the planning pass.
6. Weekly review's own context-builder/report cluster (`buildWeeklyReviewContext` and ~20 sibling functions, `server.ts:3110-3591`) — the concrete prerequisite identified by the `/messages/process` audit for ever safely extracting item 5.
