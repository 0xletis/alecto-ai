...
**Update:** both halves of this blocker are now resolved. The Gmail-rule half — see "Gmail Rule Service Extraction" below: `writeGmailAutonomyPreferences`/`buildGmailAutonomyState`/`archiveEmailSignalRule`/`createEmailSignalRule` were already clean imports; the one remaining server.ts-local dependency (`formatGmailEmailRuleSelectionLines`) is now extracted. The action-hygiene half — see "Action Hygiene Service Extraction" below: `applyActionHygieneBatchOperations` and `readActionHygieneBatchOperations` are now extracted too. `applyPendingAction`'s `custom_email_rule`, `gmail_autonomy_preference`, and `action_hygiene` branches are all fully clean now. `pending-actions` itself is next — see the recommended target list below.
...
Recommended next extraction target (in safety order, re-verified during the action hygiene service pass):

1. **`pending-actions` route group is now safe to extract.** `applyPendingAction`'s three previously-tangled branches (`custom_email_rule`, `gmail_autonomy_preference`, `action_hygiene`) are all fully clean as of this pass — see "Action Hygiene Service Extraction" and "Gmail Rule Service Extraction" below. This is the recommended next pass.
...
**Next cleanup target:** `pending-actions` route group extraction itself (`GET /users/:userId/pending-actions`, `POST .../confirm`, `POST .../reject`, plus `applyPendingAction` and `findPendingAction`) — now safe, per the Server.ts Map's recommended order above.
