...
| Gmail review inbox | implemented | `/email_reviews`, natural `email reviews`, `show 1`, `approve 1`, `reject 2`, `reject all the rest reviews from Endesa`, `turn 3 into an action tomorrow` | EmailReviewItem | optional classifier/router | V3 Gmail alerts for opted-in users; legacy bundled scheduled review-waiting notification remains fallback | richer review UX, web UI |
