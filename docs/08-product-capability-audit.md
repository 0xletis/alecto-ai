...
| Goals | implemented | `/goals`, `/create_goal`, `/archive_goal`, natural goal creation, natural archive/pause (`archive this goal`, `pause this goal`, `stop tracking X`, Spanish/Catalan equivalents; ambiguous names ask instead of guessing, confirmed before applying) | Postgres `Goal` | optional classification | no | goal editing (title/why/signals) through chat not yet supported |
...
| Gmail readonly | partial | `/connect_gmail`, natural Gmail connect/reconnect/status link, natural Gmail setup/status/list/sync/rule enable/custom tracking, natural alert/autonomy preferences, multilingual pending and active custom-rule edits/questions/timing answers, sync-debug questions, bulk custom-rule removal confirmations, `/sync_gmail` | Gmail API | optional semantic router/classifier plus adaptive active-rule matcher over safe metadata/snippet fields, with deterministic filters/dedupe/executors | manual sync or explicit per-user scheduled background sync when the worker master switch is enabled; review notifications through legacy bundle or V3 Gmail alerts when eligible | production OAuth/account management, key rotation, broader real-user evals |
| Gmail review inbox | implemented | `/email_reviews`, natural `email reviews`, `show 1`, `does the jobs newsletter mention frontend roles?`, `approve 1`, `reject 2`, `ignore 1`, `keep 4 in review`, `reject all the rest reviews from Endesa`, `turn 3 into an action tomorrow`, `turn 2 and 3 into tasks 5 minutes from now and remind me at that time`, `do I have any reminders on` | EmailReviewItem + explicit ActionItems/reminder ActionItems | optional classifier/router | V3 Gmail alerts for opted-in users; legacy bundled scheduled review-waiting notification remains fallback | richer review UX, web UI |
...
- `Gmail status`
- `reconnect Gmail`
- `send me the Gmail reconnect link`
- `integrate email`
...
- `remove Endesa rule`
- `sync Gmail`
- `why did Gmail sync find nothing?`
- `sync integrations`
...
- Can it read mail independently? **Partially.** Gmail can sync readonly after OAuth and explicit active email rules. Natural `show Gmail setup`, `Gmail status`, `what can Gmail track`, `what email rules are on`, `what email rules do we have`, `enable job search rule for Gmail`, `enable work action rule for Gmail`, and custom requests such as `create a rule for Endesa bills`, `track Endesa bills from Gmail`, or Spanish/Catalan equivalents explain current state and use safe rule paths. Agent Runtime v3 explicit sync phrases such as `sync Gmail`, `sync email`, `check Gmail now`, `check my email now`, `refresh Gmail`, and `look for new emails now` use the same safe manual sync path as `/sync_gmail`, not passive status; auth/token/decrypt/no-rule failures return safe actionable messages and never expose tokens, ciphertext, IVs, tags, or raw provider responses. Gmail status and rule listing now use the same active-rule source, preferring an active Gmail connection with active rules when duplicate/stale connections exist. Natural timing questions such as `when do u check my gmail?`, `do you check Gmail automatically?`, and `will you notify me about emails?` explain manual sync, scheduled sync status, review-notification preference, no instant Gmail webhooks, and review handling before any broad custom-rule path can run. Natural preference changes such as `check Gmail every hour`, `make Gmail manual only`, and `turn off Gmail review notifications` ask for confirmation before saving Gmail connection preferences. Gmail stays manual-only by default; background Gmail sync runs only when the global worker master switch is enabled, the user explicitly chose scheduled checks, Gmail has at least one active rule, and the per-connection interval is due. Manual `sync Gmail` does not reset the background due time. If worker scheduling is globally disabled, Alecto says the preference is saved but background checks are not active locally. Daily digest and work-hours checks are recognized but not implemented. No active rule means no scanning. Custom sender/keyword tracking is implemented as review-only v1 and requires confirmation before enabling. Pending proposals and active custom rules can be edited or questioned in natural language, including keyword replacement wording such as `looks for only Aigues de Barcelona instead of Endesa`, `busca solo Aigues de Barcelona, no Endesa`, follow-ups like `make that rule look for only Aigues de Barcelona instead of Endesa` or `pause it`, and sync/timing questions such as `when will you let me know about it?` or `quan m'avisareu dels correus d'Endesa?`. Ambiguous rule management such as `elimina Endesa` stores a pending clarification, accepts number/name replies, and still confirms before removal. Multi-target cleanup such as `elimina Aigues de Barcelona y Endesa` asks one confirmation for matched Gmail email rules. Contextual reset/delete-all phrasing after a rule list, such as `can u delete all of em?`, `turn all off and delete them`, or `delete all email rules`, asks to remove all visible Gmail email rules after confirmation and does not fall into action-control or multi-intent matching. Duplicate built-in job-search/work-action rules are grouped in natural output, ignored during sync as exact duplicates, and cleaned when the built-in rule is enabled again. `email reviews` and Spanish/Catalan pending-email phrases show a grouped short-lived review inbox with visible numbers in display order; `show 1`, `does the jobs newsletter mention frontend roles?`, `approve 1`, `reject 2`, `ignore 1`, `keep 4 in review`, `reject all the rest reviews from Endesa`, `turn 3 into an action tomorrow`, `turn 2 and 3 into tasks 5 minutes from now and remind me at that time`, and `turn 2 and 3 into tasks at the time they say and remind me 30 minutes before each` operate only on still-pending visible reviews/tasks. Explicit mixed review triage is reconciled before execution, so ignored/kept numbered reviews cannot also become tasks. Reminder queries such as `do I have any reminders on` list reminder ActionItems only. Review-to-task reminders are real companion `ActionItem` rows, not Gmail or Calendar mutations. Security/auth/account emails are hard-filtered before unrelated job/work/custom review creation, but explicit security/account-alert rules can match them. Scheduled sync can send one bundled legacy "Gmail reviews are waiting" notification when new review items are created and the Gmail notification preference allows it; if Agent Runtime V3 Gmail alerts are live and eligible (`gmailNudgeEnabled`, delivery flag, allowlist, quiet-hours/dedupe), V3 sends the alert instead and the legacy bundle is suppressed to avoid duplicates. The Gmail connection and historical email reviews/events are kept. Utility/bill tracking avoids false links to the health `energy` goal unless a real utility/expense goal exists.
- Can normal chat handle Gmail reconnect/setup? **Yes.** Agent Runtime v3 owns natural `connect Gmail`, `connect my email`, `integrate email`, `reconnect Gmail`, and `send me the Gmail reconnect link` requests. It reuses the real `/connect_gmail` OAuth URL builder, shows expired auth as a reconnect action first, and never syncs Gmail from these status/link requests. `turn on Gmail alerts`, `tell me when important emails arrive`, and `avísame de correos importantes` are confirmed notification-setting changes only; if Gmail is expired or disconnected, Alecto enables the preference only after confirmation and says reconnect/connect is required before alerts can work.
- Can it ask for updates alone? **Yes.** Worker can send daily check-ins, daily insights, weekly insights, and daily loop briefs.
- Can it create actions from email? **Yes, with review.** `work_action_email` creates review items; approval creates ActionItems. Agent Runtime v3 can also convert visible Gmail reviews into grounded ActionItems on explicit request, parse simple user/email timing, create companion pre-due reminder ActionItems, and correct the created task time without duplicating the task. It still does not create Calendar events or mutate Gmail.
...
- Gmail sync is still deterministic query/rule-first, but now has an optional adaptive AI rule-matching pass over a bounded recent-message metadata/snippet slice for messages not already processed. The matcher receives only safe fields plus active Gmail rule definitions and relevant active goals, must return a real active rule id/name or a skip reason, and creates EmailReviewItems only. This covers work actions, recruiter/job-search, bills/invoices, security/account alerts, and custom rules without hardcoding the downstream architecture around work email.
- Zero-new-review sync replies now point to `why did Gmail sync find nothing?`; Agent Runtime v3 answers that via `gmail.sync.debug`, a read-only last-sync diagnostic summary that does not run another sync and does not expose tokens, full bodies, or provider payloads.
- A good next step is making review handling and setup feel more guided without increasing email-side automation.
- Architecture boundary: do not design Gmail alerts as a one-off closed system. The long-term shape is `integration observation -> goal/action/calendar/memory/evidence candidate -> V3 tool/action`. `EmailReviewItem` remains fine as the current Gmail source object, but downstream product decisions should be generic operations such as create action, log goal evidence, ask clarification, or later suggest a calendar event. Calendar is not implemented and no fake Calendar tools should be added yet; document where it would plug in instead.
