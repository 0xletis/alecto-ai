...
| Gmail readonly | partial | `/connect_gmail`, natural Gmail setup/status/list/sync/rule enable/custom tracking, natural autonomy preferences, multilingual pending and active custom-rule edits/questions/timing answers, bulk custom-rule removal confirmations, `/sync_gmail` | Gmail API | optional semantic router/classifier with hard deterministic filters first | manual or scheduled sync, review notifications if enabled | production OAuth/account management, key rotation, broader real-user evals |
...
- Gmail already has OAuth, rules, review inbox, work/action adapters, custom sender/keyword review-only tracking, state-aware setup/status/timing replies, manual/scheduled sync preference, review-notification preference, and bundled review-waiting notifications on scheduled sync.
