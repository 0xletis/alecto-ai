...
import { buildWeeklyReviewContext, getLatestWeeklyReview } from "../legacy/weekly-review-conversation.js";

/**
 * Reusable, non-legacy next-week/current-week planning logic: plan-context
 * assembly, plan-suggestion generation (deterministic + optional LLM), and
 * the pure formatting/dedup helpers built on top of them. Split out of
 * apps/api/src/legacy/planning-conversation.ts (which re-exports everything
 * here for its existing callers) as part of the Next-Week Planning V3
 * migration, so Agent Runtime v3 can build/show/apply a next-week plan
 * without importing the legacy PendingAction-coupled reply parser
 * (parseNextWeekPlanReply/resolveNextWeekPlanReply/replacePendingPlan, which
 * remain legacy-only). See docs/09-architecture-inventory.md's "Next-Week
 * Planning V3 Migration" for the full boundary rationale.
 *
 * buildNextWeekPlanContext still transitively imports
 * legacy/weekly-review-conversation.ts (buildWeeklyReviewContext) because
 * weekly-review context assembly itself has not been extracted out of
 * legacy/ — that remains a future migration candidate, not in scope here.
 */
