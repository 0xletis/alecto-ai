import { isRiskControlGoal, type MemoryEntry } from "@operator-agent/core";
import { createMemory, getActiveMemories, getMemories, updateMemory } from "@operator-agent/db";
import type { WeeklyEmailAttentionSummary, WeeklyReviewContext, WeeklyReviewDraft, WeeklyReviewMemory } from "../server-types.js";
import { formatDateInTimezone, startOfLocalWeek } from "../utils/datetime.js";
...
import { countType, weeklyContextCounts } from "./context.js";

/**
 * Reusable, non-legacy weekly-review GENERATION/FORMATTING/SAVING built on top of an
 * already-built WeeklyReviewContext (apps/api/src/weekly-review/context.ts). Split out of
 * apps/api/src/legacy/weekly-review-conversation.ts (which re-exports everything here for its
 * existing callers) as part of the Weekly-Review V3 Migration.
 */
