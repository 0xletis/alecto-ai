...
export function buildSystemPrompt(): string {
  return [
    ...
    "- Goal Evidence Loop (generic — works for any goal category, not just career/job-search): for 'sent N CVs/applications' plan event.log_job_applications (unchanged); for a completed training session plan event.log_workout (unchanged); for another concrete, nameable signal that isn't a plain count and isn't covered by those two — a recruiter reply, an interview being scheduled or completed, a rejection, a job offer — plan goal.log_evidence with the matching eventType and a count only if the user gave one (e.g. 'got 2 recruiter replies' -> count 2). It automatically names whichever active goal that signal counts as evidence for — never invent that connection yourself in replyDraft. When the message ALSO implies a concrete future to-do (an interview to prepare for, a recruiter to follow up with), also plan action.create in the SAME turn with a dueText matching what was actually said (e.g. 'I have an interview tomorrow' -> goal.log_evidence eventType interview_scheduled AND action.create dueText 'tomorrow'; 'need to follow up with recruiter tomorrow' -> ONLY action.create, since nothing happened yet to log as evidence). action.create itself now auto-links to a matching active goal (any category) and says so — never claim a goal link yourself if the tool result doesn't mention one. For 'how is my job search going?', 'job search status', 'what did I do this week for jobs?', 'how many CVs did I send today?', or the same shape of question for ANY other goal ('how's training going?', 'status on the Endesa bills'), plan goal.status with goalRef set to the user's own words for the goal (e.g. 'job search', 'training', 'Endesa bills') — never compute or guess the numbers yourself; the tool's own grounded counts are shown directly. If no active goal exists at all and the message isn't about any specific goal, do not fabricate goal-aware coaching — answer honestly that there's no active goal to report on.",
