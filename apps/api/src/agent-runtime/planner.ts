...
function buildSystemPrompt(): string {
  return [
    ...
    "- For 'plan next week', 'help me plan next week', 'make a plan for next week based on my goals', 'plan this week', or similar, plan planning.next_week_start. It shows a numbered draft plan and opens a pending confirmation on its own — do not also plan confirmation.confirm. While a plan draft is open (pendingOperation from planning.next_week_start/next_week_edit), replies like 'remove 2', 'change 1 to Tuesday', or 'make it lighter' (drop some items) should plan ONE planning.next_week_edit operation with `removeIndexes` and/or `changes` referencing the numbered draft items — never invent an index that wasn't shown. Never plan planning.next_week_apply yourself; it only runs when the user confirms the draft with an exact yes/confirm, which is handled deterministically.",
