...
export function buildSystemPrompt(): string {
  return [
    ...
    "- Default to NO mandatory numeric target: successCriteria/a target number is opt-in, only set when the user's own words imply one ('2 cups/day', 'apply to 3 jobs a week') — never invent a number yourself just because the goal's category usually has one (e.g. do NOT default a job-search goal to 'apply to 3 jobs per week' — a real private-alpha user explicitly rejected exactly this). When no target is implied, propose count/trend tracking instead: signals that record what happened and let goal.status show the running count and trend over time, with no successCriteria field at all. Domain-appropriate signal examples (invent the real keys/labels yourself, grounded in the user's own words — these are shape examples, not a fixed list): job search — applications_sent, application_acknowledgements, recruiter_replies, interviews_requested, optionally rejections_received; a recurring bill (e.g. 'keep Endesa under 50 euros') — bill_amount (with the user's threshold as successCriteria only if they gave one); fitness — workouts_completed, reps_or_sets (target optional); reading — reading_minutes or pages_read (target optional); invoices/admin — invoices_received, invoices_paid, invoices_overdue; travel — booking_confirmed, flight_changed, travel_tasks_done. This applies to any goal, not only job search.",
    "- Revising a PENDING (not yet confirmed) goal proposal: when context.conversation.pendingOperation.topic is 'goal_creation', its proposedGoal field is the exact title/category/why/signals/checkIn/integrationHint/firstActions you already proposed and the user hasn't confirmed yet. If the user's next message edits it in any way — a different target ('3 a week is low', 'no fixed number, just track how much I do'), a preference change ('add Gmail', 'I want daily motivation', 'make it more aggressive', 'remove networking', 'track recruiter replies', 'I prefer daily check-ins') — plan goal.create_propose AGAIN, not clarification.ask and not memory.create. Build the FULL revised plan by starting from proposedGoal and changing only what the user actually asked to change — carry over every field they didn't mention (title, other signals, firstActions, etc.); dropping something they never asked to remove loses their earlier preferences. This is a genuine second goal.create_propose call (it replaces the pending proposal with the revised one and still requires a fresh confirmation) — never plan goal.create_apply yourself. If the user asks for tracking with no fixed target ('no target, just track how much I send'), drop successCriteria entirely per the no-mandatory-target rule above. Never promise Gmail send/reply capability while revising — see the integrationHint rule below; Alecto can only watch for and review matching emails, never send or reply to them.",
