...
export const toolCatalog: ToolDefinition[] = [
  ...
  },
  {
    ...
    argsSchema: z.object({
      ...
      dueText: z
        ...
        .optional()
        .describe(
          "Natural language due date/time, e.g. 'tomorrow', 'friday at 5pm', 'tomorrow at 20:00'. NEVER compute or pass a raw ISO-8601 timestamp (e.g. '2026-09-01T20:00:00Z') or any other pre-calculated date string yourself — the deterministic parser (which already knows the user's real local date/timezone) does that calculation, correctly, every time; a self-computed UTC timestamp risks landing on the wrong local hour or the wrong day entirely. Pass the date/time phrase in words, close to how the user said it. If the user stated a weekday ALONGSIDE a day-of-month (e.g. 'Thursday 26 August', 'jueves 26 de agosto', 'dijous 26 d'agost'), pass BOTH exactly as said — never drop or 'correct' the weekday yourself, even if it looks inconsistent with the day number. The deterministic parser checks that the two actually match on the real calendar and asks the user to clarify if they don't; silently dropping one side here would hide a real contradiction instead of catching it."
        ),
      goalId: z
        .string()
        .optional()
  ...
  },
  {
    name: "action.archive",
    description:
      "Archive a SINGLE, specifically-named or specifically-numbered action item (dismiss without completing — never imply it was finished). Use for 'archive it', 'drop it', 'remove it', 'no longer needed', 'not needed anymore' (English); 'archívalo', 'bórralo', 'ya no hace falta', 'elimínalo' (Spanish); 'arxiva-ho', 'ja no cal', 'esborra-ho' (Catalan) — a bare phrase like these refers to whichever task is currently in view, same resolution as action.complete above (omit actionId). When exactly two visible actions are near-duplicates of each other (same real content, different date wording) and the user says 'archive the duplicate'/'archive one of them'/'keep the first' (or 'keep 1'/'remove 2', naming the other one explicitly), the target is whichever one ISN'T being kept — use its real visible-entity id, never invent one. This tool works the same way for a currently OPEN or a currently DEFERRED/moved action — never treat 'not open yet' as a reason to decline. Never use this for 'all'/'these'/'them' — see action.archive_all_propose for that.",
    mutates: true,
    requiresConfirmation: false,
    argsSchema: z.object({ actionId: actionIdField })
  },
  ...
  },
  {
    ...
    description:
      "Change an existing action item's due date/time. Alecto actions are commitments — open, completed, or archived, nothing else — so this ALWAYS updates dueAt and keeps the action OPEN and visible in every normal action list; it never completes, archives, or hides the action, and never creates a duplicate. This is the ONLY tool for moving/postponing/reminding-later language, in both directions: pushing a due date LATER ('move it to tomorrow', 'move it to tomorrow 20:00', 'reschedule it to tomorrow', 'set it for tomorrow', 'change it to tomorrow', 'do it tomorrow', 'do it later', 'bring it back tomorrow', 'remind me tomorrow', 'not now, tomorrow', 'later today at 20:00', 'park it until Friday', 'push it to tomorrow', 'push it back', 'defer it', 'not today', 'not now' — English) AND pulling a date back EARLIER ('move it back to today', 'pull it forward', 'actually let's do it today' -> dueText 'today'). 'not today'/'not now'/'do it later' alone (no explicit date) mean tomorrow, the natural default — set dueText to 'tomorrow' unless the user actually named a different day. Same in Spanish ('muévelo a mañana', 'recuérdamelo mañana', 'pásalo a mañana', 'mañana') and Catalan ('mou-ho a demà', 'recorda-m'ho demà', 'passa-ho a demà', 'demà') — a bare phrase like these refers to whichever task is currently in view or was just mentioned (omit actionId, same resolution as action.complete below). For a genuinely vague target ('later this week', 'más adelante esta semana', 'més endavant aquesta setmana') with no specific day named, do NOT guess a day yourself — this is handled deterministically before you ever see it (a real weekday clarification is asked), so if you do see concrete text like 'thursday' or 'friday' here it's because the user (or the clarification's own answer) already named a real day; use it as given. Use for corrections too, like 'brainstorm meeting means 12pm not 12am, change it' or 'move the YouTube task to tomorrow afternoon'. Prefer actionId from visible context; otherwise pass ref using the task's visible wording — the ref resolver searches open AND any legacy deferred/snoozed actions by title, not just what's currently on screen, so 'do I already have an action to send CVs? move it to today if so' can be answered in one call: plan action.reschedule with ref 'send CVs' and dueText 'today' even if nothing about sending CVs is currently visible. If it genuinely doesn't exist, the tool itself asks honestly and offers to create one instead — never plan action.create yourself for a 'do I have X' question; let the reschedule attempt answer it first. Never plan action.snooze for any of this — that tool is deprecated for normal goal actions and is not offered.",
    mutates: true,
    requiresConfirmation: false,
    argsSchema: z.object({
      ...
      dueText: z
        ...
        .optional()
        .describe(
          "A full natural-language due date/time, e.g. 'tomorrow afternoon', 'tomorrow at 20:00'. NEVER compute or pass a raw ISO-8601 timestamp (e.g. '2026-09-01T20:00:00Z') or any other pre-calculated date string yourself — the deterministic parser (which already knows the user's real local date/timezone) does that calculation, correctly, every time; a self-computed UTC timestamp risks landing on the wrong local hour or the wrong day entirely. Pass the date/time phrase in words, close to how the user said it. If the user stated a weekday ALONGSIDE a day-of-month (e.g. 'Thursday 26 August', 'jueves 26 de agosto', 'dijous 26 d'agost'), pass BOTH exactly as said — never drop or 'correct' the weekday yourself, even if it looks inconsistent with the day number. The deterministic parser checks that the two actually match on the real calendar and asks the user to clarify if they don't; silently dropping one side here would hide a real contradiction instead of catching it."
        ),
  ...
  },
  {
    ...
    description:
      "The PRIMARY, goal-driven way to turn Gmail on for one goal — propose using Gmail readonly to watch for signals relevant to a specific active goal, e.g. 'use Gmail for my job search', 'watch my Gmail for this trip', 'can you use my email for the insurance goal', or the turn IMMEDIATELY after a new goal is created for a goal that's obviously email-relevant (job search, travel, insurance, car/vehicle, bills/admin, work/client projects) — not any later, unrelated turn just because that goal happens to still be active. Infers what to watch FROM the goal itself — the user never needs to describe rule config, keywords, domains, or classifier settings. If Gmail is not connected, says so and gives the connect link instead of proposing a watcher. If a watcher already covers this goal, says so instead of proposing a duplicate. If the goal has no obvious email signal (fitness, reading, a personal habit), says so honestly instead of forcing one — do not plan this tool for a goal like that just because the user asked broadly ('use Gmail more'); ask what specifically first. A message merely being ABOUT the goal's domain (e.g. mentioning job-search vocabulary) is never enough on its own — the message itself must actually say something about Gmail/email/mail/watching/syncing. A greeting ('gm', 'GM', 'good morning', 'morning') is NEVER Gmail intent, no matter what job-search-relevant language follows it in the same message — a real reported bug had 'Gm will send anything web3 dev that fits my style' (replying to a morning coaching brief) misread this way. Opens a pending confirmation; never enables anything silently.",
    mutates: false,
    requiresConfirmation: false,
    opensPendingProposal: true,
    argsSchema: z.object({
