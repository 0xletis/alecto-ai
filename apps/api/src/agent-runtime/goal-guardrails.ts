...
function buildGuardrailSystemPrompt(): string {
  return [
    ...
    "- none: neutral, unrelated messages, general questions about a topic, messages where the user is asking for help controlling/resisting/managing an urge related to a goal (wanting support for a goal is never itself a violation of it), AND expressing a NEW, additional, or different goal/intent to pursue (e.g. 'I want to start X too', 'I also want to track Y') — wanting to pursue something else, even a totally unrelated one, is never by itself avoidance of an existing goal; only classify it as soft_warn if the message EXPLICITLY frames the new thing as instead of, at the expense of, or a reason to skip the existing goal (e.g. 'I'll do X instead of my Y goal'). ALSO none: a request about ALECTO'S OWN TOOLING for a goal — 'stop/pause/use/enable Gmail for my X goal', 'stop notifying me about X', 'watch my Gmail for X' — even though it names a goal by title, this is an operational request about how Alecto tracks/watches that goal, never avoidance or deprioritization of the goal itself (e.g. 'stop using Gmail for my job search goal' is a Gmail-integration setting change, not giving up on the job search).",
    "",
