...
const directFinancialRiskPattern =
  /\b(quiero apostar|voy a apostar|i want to bet|i'?m going to bet|plan to bet|plan for betting|help me plan (?:a )?bet|what should i do(?: today)? to win (?:a )?bet|win (?:a )?bet|bet safely|place bet|make a bet|remind me to bet|add action to bet|notify me (?:when|about).*betting|track betting signals?|move (?:my )?bet|reschedule (?:my )?bet|snooze (?:my )?bet|push (?:my )?bet|mark (?:my )?(?:bet|betting) task done|betting signals?|signals? for betting|betting tips?|bet of \d+|bet\s+\d+|apuesta\s+\d+|apostar\s+\d+|quiero tradear|voy a tradear|i want to trade|plan to trade|plan my week around trading signals?|trade safely|trading signals?|signals? for trading|trading tips?|move (?:my )?trade|reschedule (?:my )?trade|snooze (?:my )?trade|mark (?:my )?trading task done|open\s+\d+x\s+(?:long|short)|\d+x\s+(?:long|short)|long|short|leverage)\b/i;
