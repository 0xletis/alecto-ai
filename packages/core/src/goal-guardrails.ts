...
const directFinancialRiskPattern =
  /\b(quiero apostar|voy a apostar|i want to bet|i'?m going to bet|place bet|make a bet|remind me to bet|add action to bet|move (?:my )?bet|reschedule (?:my )?bet|snooze (?:my )?bet|push (?:my )?bet|mark (?:my )?(?:bet|betting) task done|bet of \d+|bet\s+\d+|apuesta\s+\d+|apostar\s+\d+|quiero tradear|voy a tradear|i want to trade|move (?:my )?trade|reschedule (?:my )?trade|snooze (?:my )?trade|mark (?:my )?trading task done|open\s+\d+x\s+(?:long|short)|\d+x\s+(?:long|short)|long|short|leverage)\b/i;
