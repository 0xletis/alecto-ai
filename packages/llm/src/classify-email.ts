...
function buildEmailClassifierPrompt(allowedEventTypes: readonly string[]): string {
  ...
  return [
    ...
    "Do not classify an email as a job offer (career.offer_received) unless it explicitly offers a job, a role, or an interview/next-step invitation — a generic 'opportunity', a job-board/newsletter listing, or a recruiter's cold outreach with no concrete offer is NOT a job offer; classify those as needs_review (recruiter outreach) instead, never offer.",
    ...
    "LinkedIn/job-platform ACCOUNT or PROFILE status notifications (e.g. 'you are no longer showing recruiters you're open to work', 'your Open to Work status is now visible', profile-visibility or privacy-setting changes) are not job-search progress of any kind — they are about the user's own account settings, not a real application/recruiter/offer event. Use decision ignore and eventType null for these, even though the wording mentions job searching.",
    "An automated 'your application was viewed' status ping (e.g. 'Okify ha visto tu solicitud', 'Iqana ha visto tu solicitud', 'your application was viewed', 'a recruiter viewed your application') is informational only — no human replied, nothing was submitted, no interview or offer happened. Use decision needs_review, eventType null, and set reason to exactly 'application_viewed' (that literal string) — never classify it as a recruiter reply, an interview, an offer, or an application confirmation.",
    "A job-board/platform DIGEST or ALERT listing multiple open roles/postings for the user to browse ('new jobs matching your search', 'jobs you may be interested in', 'recommended for you', a weekly/daily roles digest) is a broadcast, not something the user did or a real recruiter/employer event — use decision ignore and eventType null even though it's clearly job-search-topical, unless it also contains a genuine, specific update about an application the user actually submitted.",
